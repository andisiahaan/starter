<div class="flex flex-col bg-white dark:bg-dark-elevated rounded-lg overflow-hidden">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-dark-border">
        <h3 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Delete Permission') }}</h3>
        ...
        <div class="flex items-start gap-4">
            ...
            <div>
                <p class="text-slate-700 dark:text-slate-300">
                    Are you sure you want to delete the permission <strong class="text-slate-900 dark:text-white">{{ $permission?->name }}</strong>?
                </p>
                @if($permission && $permission->roles_count > 0)
                <p class="mt-1 text-sm text-amber-600 dark:text-amber-400">
                    Warning: This permission is assigned to {{ $permission->roles_count }} role(s).
                </p>
                @endif
