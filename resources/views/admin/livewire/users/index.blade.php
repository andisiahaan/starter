                ...
                <div class="overflow-hidden shadow ring-1 ring-slate-200 dark:ring-dark-border md:rounded-lg">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-dark-border">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-dark-border bg-white dark:bg-dark-base">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-dark-elevated transition-colors {{ in_array($user->id, $selected) ? 'bg-primary-50 dark:bg-primary-900/10' : '' }}">
                                ...
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 flex-shrink-0 rounded-full {{ $user->isBanned() ? 'bg-red-100 dark:bg-red-900/30' : 'bg-primary-100 dark:bg-primary-900/30' }} flex items-center justify-center">
                                            <span class="{{ $user->isBanned() ? 'text-red-600 dark:text-red-400' : 'text-primary-600 dark:text-primary-400' }} font-medium">{{ strtoupper(substr($user->name, 0, 2)) }}</span>
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                    <button wire:click="$dispatch('openModal', { component: 'admin.users.modals.create-edit-user-modal', arguments: { userId: {{ $user->id }} } })" class="text-primary-600 dark:text-primary-400 hover:text-primary-500 dark:hover:text-primary-300 mr-2">Edit</button>
                                    @if($user->id !== auth()->id())
                                    @if($user->isBanned())
                                    <button wire:click="unbanUser({{ $user->id }})" class="text-green-600 dark:text-green-400 hover:text-green-500 dark:hover:text-green-300 mr-2">Unban</button>
                                    @else
                                    <button wire:click="$dispatch('openModal', { component: 'admin.users.modals.ban-user-modal', arguments: { userId: {{ $user->id }} } })" class="text-orange-600 dark:text-orange-400 hover:text-orange-500 dark:hover:text-orange-300 mr-2">Ban</button>
                                    @endif
                                    @endif
                                    <button wire:click="$dispatch('openModal', { component: 'admin.users.modals.delete-user-modal', arguments: { userId: {{ $user->id }} } })" class="text-red-600 dark:text-red-400 hover:text-red-500 dark:hover:text-red-300">Delete</button>
                                </td>
                            </tr>
                            @empty
                            <tr>
