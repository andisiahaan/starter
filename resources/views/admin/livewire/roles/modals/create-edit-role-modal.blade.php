<div class="flex flex-col bg-white dark:bg-dark-elevated rounded-lg overflow-hidden">
    ...
    <div class="p-5">
        <form wire:submit="save" class="space-y-4">
            ...
            <div>
                <label for="guard_name" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Guard Name</label>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto p-3 bg-slate-50 dark:bg-dark-soft rounded-md border border-slate-200 dark:border-dark-border">
                    ...
                    <label class="flex items-center gap-2">
                        <input type="checkbox" wire:model="selectedPermissions" value="{{ $permission->name }}" class="rounded border-slate-300 dark:border-dark-border text-primary-600 focus:ring-primary-500 bg-white dark:bg-dark-muted">
                        <span class="text-sm text-slate-700 dark:text-slate-300">{{ $permission->name }}</span>
                    </label>
                    @endforeach
                </div>
            </div>
