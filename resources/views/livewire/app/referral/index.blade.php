<div>
        ...
        <div class="px-6 py-8 sm:px-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-4xl font-bold text-white mt-1">Rp {{ number_format($stats['total_earnings'], 0, ',', '.') }}</p>
                    <p class="text-emerald-200 text-sm mt-1">From {{ $stats['total_referrals'] }} referrals</p>
                </div>
            ...
            <div class="mt-6 flex gap-3">
                <a href="{{ route('app.referral.withdrawals') }}" class="inline-flex items-center px-4 py-2 bg-white/20 text-white text-sm font-medium rounded-lg hover:bg-white/30 transition-colors">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="bg-white dark:bg-dark-elevated rounded-xl shadow-sm border border-slate-200 dark:border-dark-border p-4">
            ...
            <p class="text-2xl font-bold text-slate-900 dark:text-white mt-1">{{ $stats['total_referrals'] }}</p>
        </div>
        <div class="bg-white dark:bg-dark-elevated rounded-xl shadow-sm border border-slate-200 dark:border-dark-border p-4">
            <p class="text-sm text-slate-500 dark:text-slate-400">Available</p>
            <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">Rp {{ number_format($stats['available_commission'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white dark:bg-dark-elevated rounded-xl shadow-sm border border-slate-200 dark:border-dark-border p-4">
            <p class="text-sm text-slate-500 dark:text-slate-400">Pending</p>
            <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400 mt-1">Rp {{ number_format($stats['pending_commission'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white dark:bg-dark-elevated rounded-xl shadow-sm border border-slate-200 dark:border-dark-border p-4">
            <p class="text-sm text-slate-500 dark:text-slate-400">Withdrawn</p>
            <p class="text-2xl font-bold text-slate-600 dark:text-slate-400 mt-1">Rp {{ number_format($stats['withdrawn_commission'], 0, ',', '.') }}</p>
        </div>
    </div>
            ...
            <div class="px-5 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full 
                        @if($commission->status === 'available') bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400
                        @elseif($commission->status === 'pending') bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400
                        @elseif($commission->status === 'withdrawn') bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400
                    ...
                    <div>
                        <p class="text-sm font-medium text-slate-900 dark:text-white">
                            Commission from {{ $commission->referredUser?->name ?? 'Unknown' }}
                        </p>
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ $commission->created_at->format('M d, Y H:i') }}</p>
                    </div>
                </div>
